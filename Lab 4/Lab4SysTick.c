...
#define HALF_SEC 					  		(8000000)  // reload value for 500ms. Reload_Value = Time_Wanted_ms *  16Mhz
...
void SysTick_Wait_HalfSec(void){
	...
	NVIC_ST_RELOAD_R = HALF_SEC - 1;  // Reload value = 8,000,000 for 500ms delay.
	NVIC_ST_CURRENT_R = 0;					  // any value clears current  
	NVIC_ST_CTRL_R |= NVIC_ST_CTRL_ENABLE;		// Enable SysTick timer
	while((NVIC_ST_CTRL_R & NVIC_ST_CTRL_COUNT) == 0) {} // wait all 8,000,000 counts until count flag is done
