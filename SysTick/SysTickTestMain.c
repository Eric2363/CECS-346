...
#define HALF_SEC 					  		(160000)  // reload value for generating 10ms time interval for 16 MHz system clock.
...
void SysTick_Wait_HalfSec(void){
	NVIC_ST_RELOAD_R = HALF_SEC - 1;  // wait 160000 counts
	NVIC_ST_CURRENT_R = 0;					  // any value clears current  
	NVIC_ST_CTRL_R |= NVIC_ST_CTRL_ENABLE;		// Enable SysTick timer
	while((NVIC_ST_CTRL_R & NVIC_ST_CTRL_COUNT) == 0) {} // wait all 160000 counts until count flag is done
...
void Wait_N_Half_Sec( uint32_t n_half_s) {
	NVIC_ST_RELOAD_R = n_half_s * HALF_SEC - 1;  // wait 160000 counts * number of times you want a 0.5s delay
	NVIC_ST_CURRENT_R = 0;					  // any value clears current  
	NVIC_ST_CTRL_R |= NVIC_ST_CTRL_ENABLE;		// Enable SysTick timer
	while((NVIC_ST_CTRL_R & NVIC_ST_CTRL_COUNT) == 0) {} // wait all 160000 counts until count flag is done
		NVIC_ST_CTRL_R &=~ NVIC_ST_CTRL_ENABLE;						// disable SysTick timer
